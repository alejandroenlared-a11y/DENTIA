...
- Working tree limpio salvo la hoja de ruta reescrita y este HANDOFF (sin commitear — el usuario no confirmó commit).
...
3. Orden de ataque inmediato (propuesto al usuario; pendiente su "adelante" explícito):
   ...
   - **Claude**: migraciones Prisma versionadas (deuda §8.1 — `build` hace `prisma db push` contra Neon producción, riesgo de pérdida de datos).
...
- **Migración a Prisma versionado: HECHA en local (19 jul, sesión 2).** Pendiente solo el paso de producción (Neon) y el commit.
...
- `package.json`: `build` ahora es `prisma generate && next build` — **`db push` eliminado del build**. Riesgo §8.1 (pérdida de datos en cada deploy) cerrado.
