...
import {
  evaluateClaraConversations,
  type ClaraConversationFixture,
  type ClaraEvaluationResult
} from "../src/lib/agent/clara-evaluation";
...
function loadFixtures(dir: string): ClaraConversationFixture[] {
...
function printReport(result: ClaraEvaluationResult) {
  const failed = result.conversations.filter(conversation => conversation.failed.length > 0);
  const byCategory = new Map<string, { earned: number; total: number; count: number }>();

  for (const conversation of result.conversations) {
    const current = byCategory.get(conversation.category) ?? { earned: 0, total: 0, count: 0 };
    current.earned += conversation.earned;
    current.total += conversation.total;
    current.count += 1;
    byCategory.set(conversation.category, current);
  }

  console.log(`Clara evaluation: ${result.score}/100`);
  ...
  for (const [category, data] of [...byCategory.entries()].sort(([a], [b]) => a.localeCompare(b))) {
...
function writeMarkdownReport(result: ClaraEvaluationResult, path: string) {
  const failed = result.conversations.filter(conversation => conversation.failed.length > 0);
  const byCategory = new Map<string, { earned: number; total: number; count: number }>();
...
function writeJsonReport(result: ClaraEvaluationResult, path: string) {
  mkdirSync(dirname(path), { recursive: true });
