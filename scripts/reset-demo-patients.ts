...
async function main() {
  const requestedSlug = process.env.DEFAULT_TENANT_SLUG || DEFAULT_TENANT_SLUG;
  const tenant =
    (await prisma.tenant.findUnique({ where: { slug: requestedSlug } })) ??
    (await prisma.tenant.findFirst({ orderBy: { createdAt: "asc" } }));

  if (!tenant) {
    throw new Error("No hay tenant disponible para resetear pacientes.");
  }
  ...
  const result = await prisma.$transaction(
    async tx => {
      ...
      const createdPatients: Array<{ id: string; name: string; phone: string; status: PatientStatus }> = [];
      ...
      for (const [index, item] of demoPatients.entries()) {
        const patient = await tx.patient.create({
          data: {
            ...
            lastVisitAt: item.lastVisitOffset === null ? null : daysFromNow(item.lastVisitOffset, 11),
            notes: item.notes
          }
        });
        ...
        await tx.invoice.create({
          data: {
            ...
            fiscalHash,
            previousFiscalHash,
            qrPayload: buildQrPayload(tenant.taxId ?? "", number, issuedAt, item.invoice.amountCents),
...
function buildFiscalInvoiceHash(input: {
  tenantId: string;
  number: string;
  issuedAt: Date;
  amountCents: number;
...
function buildQrPayload(issuerTaxId: string, number: string, issuedAt: Date, amountCents: number) {
  const date = issuedAt.toISOString().slice(0, 10);
  const amount = (amountCents / 100).toFixed(2);
  return `https://www2.agenciatributaria.gob.es/wlpl/TIKE-CONT/ValidarQR?nif=${encodeURIComponent(issuerTaxId)}&numserie=${encodeURIComponent(number)}&fecha=${date}&importe=${amount}`;
}
